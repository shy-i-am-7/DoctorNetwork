...
export default class QuestionList extends Component {
  constructor(props) {
    ...
    this.state = {
      questionPhysicianList: [], // Store all questions
      questionResidentList: [], // Store all questions
  ...
  retrieveQuestions() {
    FormDataService.getAll()
      .then(response => {
        ...
        const sortedPhysQuestions = response.sort((a, b) => {
          return parseInt(a.id) - parseInt(b.id); // Ensure comparison is numeric
        });

        // Set sorted questions to state
        this.setState({
          questionPhysicianList: sortedPhysQuestions
        });

        console.log(sortedPhysQuestions); // Log the sorted questions
      })
      .catch(e => {
      ...
      .then(response => {
        // Sort the questions by their numeric ID
        const sortedResQuestions = response.sort((a, b) => {
          return parseInt(a.id) - parseInt(b.id); // Ensure comparison is numeric
        });

        // Set sorted questions to state
        this.setState({
          questionResidentList: sortedResQuestions
        });

        console.log(sortedResQuestions); // Log the sorted questions
      })
      .catch(e => {
        console.log(e); // Log any errors
      });
  ...
  render() {
    const { questionPhysicianList, questionResidentList, currentQuestion, isPhysicianDetails } = this.state; // Removed 'currentIndex'
        ...
        <div className="questions-and-answers-container">
          <div className="box questions-list" style={{ flex: 1.5 }}>
            ...
            <div className="content changeable-font" id="questions-content">
              ...
              {questionResidentList.map((question, index) => (
                <div
                  className="item"
      ...
      return (
        <div className="questions-and-answers-container">
          <div className="box questions-list" style={{ flex: 1.5 }}>
            ...
            <div className="content changeable-font" id="questions-content">
              {/* Loop through the sorted questionList and display the questions */}
              {questionPhysicianList.map((question, index) => (
                <div
                  className="item"
                  onClick={() => this.setActiveQuestion(question)} // No need for 'index'
                  key={question.id}  // Use the question ID as the key
